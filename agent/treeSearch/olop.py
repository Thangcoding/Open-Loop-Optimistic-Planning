...
from tool.ucb_function import  upper_bound_1 
...
class OLOPTree:
    ...
    def get_plan(self):
        self.planner.plan_path = max([seq for seq in self.sequence_arm], key = lambda x : self.sequence_arm[x][1].value_sequence())
    ...
    def plan(self):
        ...
        self.planner.root = OLOPNode(self.planner)     
...
class OLOPNode:
    ...
    def update(self, reward, done):
        ...
        self.count += 1 
        self.mu = upper_bound_1(self.cumulative_reward, self.count,self.planner.config['episode']) 
...
if __name__ == '__main__':
    config = {'gamma': 0.9, 'horizon': None, 'episode': None, 'budget': 1000}
    env = GridObstacleEnv(width= 4, height= 4)
    planner = Planner(config, env)
    learner = OLOPTree(planner)
